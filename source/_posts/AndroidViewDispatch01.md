...
###   ViewGroup不拦截事件 ( onInterceptTouchEvent(ev) = false )
- 第33行　intercepted 返回false,进入58行`if (!canceled && !intercepted)`语句中
- 第89行　遍历子ViewGroup 子元素,子元素是否在播动画和点击事件的坐标是否落在子元素区域内, 作为 判断子元素能否接收到点击事件
- 第179行.dispatchTransformedTouchEvent把事件传递给子元素的dispatchTouchEvent,接着看段dispatchTransformedTouchEvent方法
...
```
 在上面的代码逻辑中child传递的不是null,因此它会直接调用子元素的child.dispatchTouchEvent(event)方法.这样事件就交由子元素处理，从而完成一轮事件分发.


###   ViewGroup拦截事件 (onInterceptTouchEvent(ev) = true)
...
```
-  intercepted = true,接着走到 163行的dispatchTransformedTouchEvent,接着进入上面代码的12行,然后到View的` dispatchTouchEvent(MotionEvent event)`,后面就会父类调用子类　ViewGroup的onTouchEvent(MotionEvent event) ，我的Demo中是rootView


-  (3)  某个View一旦决定拦截,那么这一个事件序列都只能由它来处理,并且它的OnInterceptTouchEvent不会再被调用.　
如果　dispatchTransformedTouchEvent的第13行 child.dispatchTouchEvent(event)返回true，先不考虑子元素的分发过程,那么ViewGroup方法的`dispatchTouchEvent(MotionEvent ev)`第122行的`  if (dispatchTransformedTouchEvent(ev, false, child, idBitsToAssign))`返回true,接着mFirstTouchTarget就会在下面的方法中被赋值．
```
	        newTouchTarget = addTouchTarget(child, idBitsToAssign);
            alreadyDispatchedToNewTouchTarget = true;
```
　当同一系列其他事件过来后看到了 viewgroup的dispatchTouchEvent ` if (actionMasked == MotionEvent.ACTION_DOWN
                    || mFirstTouchTarget != null)`,因为mFirstTouchTarget action_down已经赋值了，所以就不会向子元素传递了．
